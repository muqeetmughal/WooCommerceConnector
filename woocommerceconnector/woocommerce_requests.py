...
def get_woocommerce_items(ignore_filter_conditions=False):
    ...
    make_woocommerce_log(title="Check2", status="Success", method=frappe.local.form_dict.cmd, message=f"sync_from_woocommerce: {str(len(woocommerce_products))} ,RESPONSE :{response.json()}")
